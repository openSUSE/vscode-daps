...
class xrefCodeLensProvider {
	...
	_parseAsciidocXrefs(document) {
		...
		while ((match = regex.exec(text)) !== null) {
			...
			const title = match[2] || match[4] || null;
			xrefElements.push({
				lineNumber: document.positionAt(match.index).line,
				columnNumber: document.positionAt(match.index).character, // Use start of match
				match: id,
				title: title
			});
		}
...
function activate(context) {
	...
	// Creates a diagnostic collection for storing and displaying problems related to broken links.
	// Diagnostics in this collection are generated by the link checking functionality.
	const linkDiagnostics = vscode.languages.createDiagnosticCollection("links");
	context.subscriptions.push(linkDiagnostics);

	// Performs an initial diagnostic check for documents that are already open when the extension activates.
	...
	// Registers a Code Actions Provider that offers a "Quick Fix" to upgrade HTTP links to HTTPS.
	...
	/**
	 * Creates a tree view for the DocBook structure.
	 */
...
document.addEventListener('scroll', () => {
	...
	// TODO: check if all other commands are created only if related function is enabled

	/**
	 * Orchestrates the link checking process for a given document.
	 * It verifies the document's language, shows a progress indicator,
	 * and calls the appropriate language-specific link checking function.
	 * After the check, it updates the diagnostics and optionally shows a summary notification.
	...
	async function checkLinksInDocument(document, showNotifications = false) {
		...
		// TODO: remove the messaging popup in bottom right from this process
		...
		await vscode.window.withProgress({ // Display a progress indicator to the user.
			...
			// TODO: check if checkDocBookLinks and checkAsciiDocLinks can share some code
			// Update the "Problems" panel with the new diagnostics.
			linkDiagnostics.set(document.uri, diagnostics);

			// If triggered manually, show a summary of the results.
			if (showNotifications) {
	...
	/**
	 * Checks if an external URL is valid by making an HTTP/HTTPS HEAD request.
	 * A HEAD request is used to check the link's status without downloading the entire content.
	 * @param {string} link - The URL to check.
	 * @returns {Promise<{isValid: boolean, message: string}>} A promise that resolves to an object
	 * containing a boolean `isValid` and a `message` string with the status or error.
	 */
	...
	async function checkAsciiDocLinks(document, diagnostics, progress) {
		 ...
		 * 1. Bare URL: `https://...` (not preceded by a backslash to avoid escaped links).
	...
	// Builds HTML or PDF targets given a DC file.
	// @param {object} contextDCfile - URI from context command (optional).
	// @returns {boolean} - True if the build was successful, false otherwise.
	...
	// Command to build a single XML file.
	...
	// Assembles a DAPS command based on the given parameters.
	// @returns {string} - The DAPS command to be executed.
	...
	// Resolves the root ID from context, configuration, or user input.
	// @param {string} contextFileURI - Optional URI from context.
	// @param {string} DCfile - The DC file to get the target root ID from.
	// @returns {string} - The discovered root ID.
	...
	// Checks if the given path represents a text document and saves it if it is dirty.
	// @param {string} XMLfile - Path to the file on the filesystem.
...
// Creates a scroll map for the HTML preview.
...
// Compiles a list of DC files in the given directory.
// @param {vscode.Uri} folderUri - URI of a folder to get DC files from.
...
// Picks a DC file from various sources (context, configuration, or user prompt).
// @returns {string} - The name of the DC file.
...
// Decides on the build target from configuration or user input.
// @returns {string} - The build target.
...
// Resolves the active file name from either a context argument or the active editor.
